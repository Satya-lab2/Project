...
export default function ManifestPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      showToast(editingId ? 'Shipment berhasil diupdate! ✓' : 'Shipment berhasil ditambahkan! ✓');
      setShowForm(false);
      setEditingId(null);
      setForm({ ...EMPTY_FORM });
      setErrors({});
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      showToast('Shipment berhasil dihapus! ✓');
      setDeleteId(null);
      fetchShipments();
    } catch {
    ...
    <div className="min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Kelola semua pengiriman kargo — {total} total data</p>
        </div>
        <button
          onClick={openAdd}
        ...
        >
          <PlusIcon className="w-4 h-4" /> Tambah Shipment
        </button>
      </div>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="Cari No Resi, Pengirim, Penerima, Jenis Barang, Status, Kota..."
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-blue-900 text-white text-xs">
                <th className="px-4 py-3 text-left">No Resi</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={10} className="text-center py-12 text-gray-400">
                    {query ? `Tidak ada hasil untuk "${query}"` : 'Belum ada data. Klik "+ Tambah Shipment" untuk mulai.'}
                  </td>
                </tr>
              ) : (
                shipments.map(s => (
                  <tr key={s.id} className="border-t border-gray-50 hover:bg-blue-50/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-700 text-xs max-w-[120px] truncate">{s.nama_pengirim}</td>
                    <td className="px-4 py-3 text-gray-700 text-xs max-w-[120px] truncate">{s.nama_penerima}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => openEdit(s)}
                          className="p-1.5 rounded-md bg-yellow-50 hover:bg-yellow-100 text-yellow-600 transition-colors"
                          title="Edit"
                        >
          ...
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
            ...
            <div className="flex gap-1">
              <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage <= 1}
                className="px-3 py-1.5 rounded-md text-sm hover:bg-gray-100 disabled:opacity-40">‹</button>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto py-6" style={{ backgroundColor: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(2px)' }}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-2xl mx-4 relative">
            ...
            <h2 className="text-lg font-bold text-gray-800 mb-1">{editingId ? 'Edit Shipment' : 'Tambah Shipment Baru'}</h2>
            {!editingId && <p className="text-xs text-gray-500 mb-4">No Resi akan digenerate otomatis</p>}
            ...
            <div className="flex justify-between mt-5">
              <button onClick={() => { setShowForm(false); setEditingId(null); }} className="bg-gray-100 text-gray-700 px-5 py-2.5 rounded-lg hover:bg-gray-200 text-sm">
                Batal
              </button>
              <div className="flex gap-3">
                {!editingId && (
                  <button onClick={() => { setForm({ ...EMPTY_FORM }); setErrors({}); }} className="text-gray-500 px-5 py-2.5 rounded-lg hover:bg-gray-100 text-sm">
                    Reset
                  </button>
                )}
                <button onClick={handleSubmit} disabled={submitting} className="bg-blue-700 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-blue-800 text-sm disabled:opacity-60">
                  {submitting ? 'Menyimpan...' : editingId ? 'Update Shipment' : 'Simpan Shipment'}
                </button>
              </div>
            </div>
          </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(2px)' }}>
          <div className="bg-white rounded-2xl shadow-2xl px-8 py-7 max-w-sm w-full mx-4 text-center">
            ...
            <h2 className="text-lg font-bold text-gray-800 mb-2">Hapus Shipment?</h2>
            <p className="text-sm text-gray-500 mb-6">
              Yakin ingin menghapus <span className="font-bold text-red-600">{deleteName}</span>? Tindakan ini tidak bisa dibatalkan.
            </p>
